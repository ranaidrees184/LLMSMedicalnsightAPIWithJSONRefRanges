...
class BiomarkerRequest(BaseModel):
    ...
    allergy_panel: float = Field(default=10.0, description="Comprehensive Allergy Profile")
...
def get_reference(biomarker_name: str, age: int, gender: str):
    ...
    return afinal or [e for e in entries if e["gender"] == "all"]
...
def build_prompt_context(user_input: BiomarkerRequest):
    ...
    for key, val in data_dict.items():
        if key in ["age", "gender", "height", "weight", "id"]:
            continue
        if key not in ref:
            continue
        ranges = get_reference(key, age, gender)
        result[key] = {"value": val, "reference": ranges}
    return result
...
def clean_json(data: Union[Dict, List, str]) -> Union[Dict, List, str]:
    if isinstance(data, str):
        ...
        return {k.strip(): clean_json(v) for k, v in data.items() if v is not None}
    return data
...
def parse_medical_report(text: str):
    ...
    data = {
        ...
        "system_analysis": {k: "" for k in SYSTEM_TEST_KEYS},
    ...
    if exec_match:
        ...
        priority_match = re.search(r"\*\*Top 3 Health Priorities:\*\*(.*?)(?=\*\*Key Strengths:|\*\*|$)", block, re.S | re.I)
        if priority_match:
            sect = priority_match.group(1)
            priorities = re.findall(r"(?m)^\d+\.\s*(.*?)(?=^\d+\.|(?=\*\*Key Strengths:)|$)", sect)
            data["executive_summary"]["top_priorities"] = [clean_line(p) for p in priorities[:3] if clean_line(p)]

        # Key Strengths
        strengths_match = re.search(r"\*\*Key Strengths:\*\*(.*?)(?=\*\*|$)", block, re.S | re.I)
        if strengths_match:
            sect = strengths_match.group(1)
            strengths = re.findall(r"(?m)^\s*[-•*]\s+(.*?)(?=^\s*[-•*]|\*\*|$)", sect)
            data["executive_summary"]["key_strengths"] = [clean_line(s) for s in strengths if clean_line(s)]

    # System-Specific Analysis (FIXED: safe regex with escaped dash)
    sys_match = re.search(r"###\s*System[- ]Specific Analysis(.*?)((?=###)|$)", text, re.S | re.I)
    if sys_match:
        sys_block = sys_match.group(1)
        for key, name in SYSTEM_TEST_KEYS.items():
            pattern = rf"\*{1,2}\s*{re.escape(name)}\s*\*{1,2}(.*?)(?=\*{1,2}|\Z)"
            found = re.search(pattern, sys_block, re.S | re.I)
            if found:
                data["system_analysis"][key] = clean_line(found.group(1))
    ...
    if plan_match:
        ...
        for m in re.finditer(r"\*\*(.+?):\*\*\s*(.*?)(?=\*\*|###|$)", block, re.S):
            ...
            data["personalized_action_plan"][key] = value

    # Interaction Alerts
    alerts_match = re.search(r"###\s*Interaction Alerts(.*?)((?=###)|$)", text, re.S | re.I)
    if alerts_match:
        block = alerts_match.group(1)
        alerts = [clean_line(line) for line in block.splitlines() if clean_line(line)]
...
very strict rules never forget: Dont duplicate anything
